...
export const ProductCard: FC<ProductCardProps> = ({ name, price, offerPrice ,onClick, onIncrement, onDecrement, inCart, inCartQuantity}) => {
  return (
    <div className="flex flex-col">
      <span className="p-4 flex flex-col bg-light-gray rounded-[2rem] w-[200px] h-[200px] relative">
      ...
      <div className="flex items-end gap-[0.8rem] tracking-wide">
        <span className="inline-block rounded-xl bg-[#FFD95F] px-[0.8rem]">
          <p className="font-secondary font-bold">
            € <span className="!text-[1.8rem]">{offerPrice.toFixed(2)}</span>
          </p>
        </span>
        <p className="line-through !text-[1.4rem] !text-gray">€ {price.toFixed(2)}</p>
      </div>
    </div>
  )
}
